...
# Заперечення зони одразу після модифікатора: «зона НЕ .com», «крім .com».
# Повноцінний фільтр-виключення зон поки НЕ підтримується (це велика зміна —
# потрібне окреме поле запиту й підтримка у двигуні). Тому головне — НЕ дати
# хибний ПОЗИТИВНИЙ фільтр (.com замість «не .com»): краще чесно не застосувати
# зону, ніж застосувати навпаки. Це задокументований ліміт.
_ZONE_NEGATION = re.compile(r"^(?:не|ні|крім|окрім|окрем\w*|без|except|not)\b")
# Те саме заперечення, але ПЕРЕД словом «зона»: «крім зони .org», «не в зоні .com».
_ZONE_NEGATION_BEFORE = re.compile(r"\b(?:не|крім|окрім|окрем\w*|без|except|not)\s+(?:[ву]\s+)?$")
...
def _extract_zone(text: str) -> tuple[tuple[str, ...], str]:
    ...
    # Заперечення зони — до або після слова «зона» («крім зони .org», «зона НЕ
    # .com»). Не даємо хибний позитивний фільтр: зону не застосовуємо, а фразу
    # затираємо, щоб .com/.org не підхопилось деінде. Фрази скасування («зона не
    # важлива») сюди не підпадають: у них немає зони, тож find_zone_mentions нижче
    # нічого не знайде й ми повернемо текст без змін.
    ...
    if negated:
        ...
        if mentions:
            ...
            return (), _mask(text, modifier.start(), offset + last_end)
        return (), text
...
def _unrecognized_names(leftover: str) -> tuple[str, ...]:
    ...
    словесних токенів вважаємо нерозпізнаними назвами.

    Поріг довжини — 2 літери, а не 3: інакше нерозпізнаний 2-літерний код
    (напр. неіснуючий «xx» у списку) зник би мовчки. Правило «жодна названа
    країна не зникає»: чого не впізнали — те в «не впізнав», а не в нікуди."""
    tokens = re.findall(r"[a-zа-яёєіїґ]{2,}", leftover)
