...
class Aggregate:
    ...
    def low_sample(self) -> bool:
        ...
        if self.count == 0:
            return False
        samples = [s for s in (self.dr_sample, self.traffic_sample) if s > 0]
        if not samples:
            return True
...
def aggregate(donors: list[Donor]) -> Aggregate:
    ...
    return Aggregate(
        count=len(donors),
        avg_dr=round(sum(dr_values) / len(dr_values), 1) if dr_values else None,
        avg_traffic=round(sum(traffic_values) / len(traffic_values), 1) if traffic_values else None,
