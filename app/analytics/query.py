...
@dataclass(frozen=True, slots=True)
class CoverageQuery:
    """Запит-ПОКРИТТЯ: скільки донорів ТРЕБА по кожній країні — і чи вистачає їх
    на кожному з порогів трафіку.

    Це не фільтр і не окремі count'и, а ОДНА операція: ШІ лише РОЗКЛАДАЄ текст
    у цю структуру (яку країну скільки треба, які пороги перевірити), а рахує
    все детермінований рушій (analytics.coverage.run_coverage). Домени/донори
    тут не фігурують — лише коди країн, потрібні кількості й пороги трафіку.

    Навмисно НЕМАЄ поля traffic_min/traffic_max: потреба («треба 20 AU») живе
    ЛИШЕ в `needs`, порогам трафіку відведено окреме `thresholds`, і зливатися
    одне в одне їм структурно нікуди. Це головний захист від сплутування
    «потрібно N» ↔ «трафік від N»."""

    section_key: str
    """Яку базу питаємо: "magic", "mordy"."""

    needs: tuple[tuple[Country, int], ...]
    """(країна, скільки треба) — у порядку, як їх назвав користувач. Кожна
    кількість додатна; список країн без повторів."""

    thresholds: tuple[int, ...]
    """Пороги трафіку для перевірки, ЗРОСТАННЯМ, завжди з 0 на початку («всього»
    і базовий рівень для підрахунку дефіциту). Напр. (0, 20, 50)."""
