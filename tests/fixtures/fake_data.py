...
мов мова-на-нейтральних у підсумок НЕ входить — вона окремим рядком. Спільна
мова визначається ПРАВИЛОМ: основна для 2+ країн словника (en, es, de, ar, nl,
pt). Букети не перетинаються (пріоритет зона → GEO → мова).

    Франція (fr, однозначна): зона .fr=3 | мова-на-GLOBAL=0 | GEO=2 → підсумок 5
              «на зонах інших країн» = 1 (be1 на .be)
    Німеччина (de, СПІЛЬНА — de/at/ch): зона .de=6 | GEO=1 (es1) → підсумок 7
              «на нейтральних зонах» = 2 (glob1,glob2); «на інших зонах» = 2 (at1,ch1)
...
# Франція (fr) — однозначна: її мова основна лише для однієї країни, тож входить
# у підсумок (тут просто 0 донорів на GLOBAL).
FRANCE_ZONE, FRANCE_LANG, FRANCE_GEO, FRANCE_TOTAL, FRANCE_ADDENDUM = 3, 0, 2, 5, 1

# Німеччина (de) — СПІЛЬНА (німецька основна для de/at/ch), тож мовний крок НЕ в
# підсумку: підсумок = зона + GEO = 7. GERMANY_LANG (2) — це складова у split
# (glob1,glob2), яку показуємо, але в total не додаємо; вона ж — нейтральний рядок.
GERMANY_ZONE, GERMANY_LANG, GERMANY_GEO, GERMANY_TOTAL, GERMANY_ADDENDUM = 6, 2, 1, 7, 2
GERMANY_NEUTRAL = 2  # німецькою на нейтральних зонах (glob1, glob2) — НЕ в підсумку
...
MORDY_OUTLINKS_ZEROS = 1  # рівно 0 вихідних — лише m4
MORDY_AVG_OUTLINKS = 17.7  # (16+20+52+0+10+8) / 6

# Розподіл за АБСОЛЮТНОЮ кількістю заспамлених лінків (групи з нулем — не в
# списку). uk1: 0 заспамлених → «0»; m1(10),m2(5),m7(4) → «1-20»; glob(52) →
# «51-100»; m4 (0 вихідних і 0 заспамлених) за правилом «0,0» → «100+»;
# m6 (порожні) — не в групи.
MORDY_SPAM_DISTRIBUTION = (("0", 1), ("1-20", 3), ("51-100", 1), ("100+", 1))
